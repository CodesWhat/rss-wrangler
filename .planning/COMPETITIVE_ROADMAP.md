...
| Hosted SaaS offering (dogfood pilot) | ⚠️ Partial | Infra/provider decisions plus Render blueprints (`render.free.yaml`, `render.yaml`) are in repo, and self-host Docker/OrbStack smoke is now green (`npm run orbstack:smoke`) with in-container health/login checks. First live hosted deploy + telemetry run still pending. |
