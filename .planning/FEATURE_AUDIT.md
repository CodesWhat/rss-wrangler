...
| Feature | Status | Notes |
|---------|--------|-------|
| Isolation model (`tenant_id` internals + v1 single-product UX) | ⚠️ | `tenant_id` exists across auth/content tables with tenant-scoped queries, RLS policies, and request/job DB context propagation (`app.tenant_id`). V1 product direction is workspace-free auth UX (single product + per-user accounts), so remaining work is simplification: hide tenant/workspace from auth flows, keep default internal scope, and reduce operational drift from unused workspace-facing paths. |
| Hosted auth + onboarding flow | ⚠️ | Better Auth flows and onboarding are shipped with invite-token controls, email verification, role model (`user_account.role`), member status gates (`pending_approval`/`active`/`suspended`), membership policy (`tenant.membership_policy`), member management endpoints, `member_event` audit trail, and Settings Members tab. Remaining gap: remove workspace input from auth UX and complete user-account-first signup/join copy + flow cleanup. |
...
| Feature | Status | Notes |
|---------|--------|-------|
| Guided onboarding wizard (first-run) | ⚠️ | Baseline wizard shipped on Home for empty-feed accounts with setup paths (add URL, OPML import, discover), optional interests starter feeds, and AI mode opt-in. Completion state now persists server-side (`onboardingCompletedAt`) and can be reset when user reopens setup. Remaining gaps: richer topic-to-folder/filter bootstrap logic. |
